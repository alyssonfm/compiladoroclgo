...
class ElementImpl implements com.pavelvlasov.uml.Element {
	...
	ElementImpl(ModelImpl model, Element holder) {
		...
		if (holder.hasAttribute(ConstantsXML.xmiType)) {
			if (holder.getAttribute(ConstantsXML.xmiType).equals(
					ConstantsXML.umlDotDot + Constants.PACKAGE)) {
				setType(AuxiliaryFunctionsXML.filterType(holder
						.getAttribute(ConstantsXML.xmiType)));
			}
		}
		NodeList nodeList = holder.getElementsByTagName("type");
		if (nodeList.getLength() > 0) {
