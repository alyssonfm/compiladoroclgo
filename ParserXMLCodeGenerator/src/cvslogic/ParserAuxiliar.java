...
public class ParserAuxiliar {
	...
	public static Operation getOperationByName(Classifier classActual,
		...
		if (op == null) {
			...
			// System.out.println(getGeneralizations(classActual));
	...
	public static void setClasses(Model model) {
		...
		for (Object p : model.getPackages()) {
			addClasses((Package) p);
			classes.addAll(((Package) p).getClasses());
		}
		classes.addAll(model.getClasses());
	}

	private static void addClasses(Package pack) {
		if (pack != null) {
			for (Object ob : pack.getElements(Constants.PACKAGE)) {
				classes.addAll(((Package) ob).getClasses());
				addClasses(((Package) ob));
			}
		}
	}
	...
	private static Attribute getAttribute(String name,
		...
		for (Object att : attributes) {
			...
			if (((Attribute) att).getName().equals(name)) {
				...
				Classifier classAux = getClassById(((Attribute) att).getType());
				if (classAux != null) {
					if (classAux.getName().equalsIgnoreCase(name)) {
						return (Attribute) att;
					}
				}
			}
